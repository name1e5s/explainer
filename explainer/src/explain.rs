...
const _SQLITE_AFF_NUMERIC: u8 = 0x43; /* 'C' */
...
fn root_block_columns(conn: &Connection) -> Result<HashMap<i64, HashMap<i64, ColumnType>>, Error> {
    let table_block_columns: Vec<(i64, i64, String, bool)> = conn.load_all(
        cstr!(
        ...
        |row| -> anyhow::Result<_> {
            Ok((
                row.column_int64(0),
                row.column_int64(1),
                row.column_text(2).to_string(),
                row.column_int(3) != 0,
            ))
        },
    )?;

    let index_block_columns: Vec<(i64, i64, String, bool)> = conn.load_all(
        cstr!(
...
struct QueryState {
    ...
    pub result: Option<Vec<(Option<DataType>, Option<bool>)>>,
}
...
pub fn explain(conn: &Connection, query: &str) -> Result<Vec<ColumnType>, Error> {
    ...
    let program: Vec<(i64, String, i64, i64, i64, Vec<u8>)> =
        conn.load_all(&query, |row| -> anyhow::Result<_> {
            Ok((
                row.column_int64(0),
                row.column_text(1).to_string(),
                row.column_int64(2),
                row.column_int64(3),
                row.column_int64(4),
                row.column_blob(5).to_vec(),
            ))
        })?;
    ...
    while let Some(mut state) = states.pop() {
        while state.program_i < program_size {
            ...
            match &**opcode {
                ...
                OP_FUNCTION => {
                    // r[p1] = func( _ )
                    // allow single_match for future use
                    ...
                    state.result = Some(
                        (p1..p1 + p2)
                            .map(|i| {
                                ...
                                let sqltype = coltype.map(|d| d.map_to_datatype());
                                let nullable =
    ...
    let mut output: Vec<Option<DataType>> = Vec::new();
    ...
    while let Some(state) = result_states.pop() {
        ...
        if let Some(result) = state.result {
            for (idx, (this_type, this_nullable)) in result.into_iter().enumerate() {
                ...
                } else if output[idx].is_none() || matches!(output[idx], Some(DataType::Null)) {
...
mod tests {
    ...
    fn test_root_block_columns_has_types() {
        let conn = Connection::establish(cstr!(":memory:")).unwrap();
        conn.exec(
            cstr!(r"CREATE TABLE t(a INTEGER PRIMARY KEY, b_null TEXT NULL, b TEXT NOT NULL);"),
            None,
        )
        .unwrap();
        conn.exec(cstr!(r"CREATE INDEX i1 on t (a,b_null);"), None)
            .unwrap();
        conn.exec(cstr!(r"CREATE UNIQUE INDEX i2 on t (a,b_null);"), None)
            .unwrap();
        conn.exec(
            cstr!(r"CREATE TABLE t2(a INTEGER NOT NULL, b_null NUMERIC NULL, b NUMERIC NOT NULL);"),
            None,
        )
        .unwrap();
        conn.exec(cstr!(r"CREATE INDEX t2i1 on t2 (a,b_null);"), None)
            .unwrap();
        conn.exec(cstr!(r"CREATE UNIQUE INDEX t2i2 on t2 (a,b);"), None)
            .unwrap();

        let table_block_nums: HashMap<String, i64> = conn
            .load_all(
                cstr!(r"select name, rootpage from sqlite_master"),
                |row| -> anyhow::Result<_> {
                    Ok((row.column_text(0).to_string(), row.column_int64(1)))
                },
            )
            .unwrap()
            .into_iter()
        ...
        let root_block_cols = root_block_columns(&conn).unwrap();
